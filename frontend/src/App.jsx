    ...
    const fetchUserProfile = async () => {
      ...
      try {
        ...
        if (err.message && (err.message.includes('Too Many Requests') || err.message.includes('429'))) {
          setError('AniList ha bloqueado tu conexiÃ³n temporalmente (Demasiadas peticiones). Cambia tu IP (ej. reiniciar router o usar datos mÃ³viles) o espera 1 hora.');
        } else {
          setError('No se pudo cargar el perfil. Puede que el token haya expirado o sea invÃ¡lido.');
        }
...
case 'mylist': {
  ...
  return (
    <div className="app-layout">
      ...
      {/* PWA UPDATE BANNER */}
      {showUpdateBanner && (
        <div className="update-banner" style={{ flexDirection: "column", gap: "0.75rem", padding: "1rem 1.5rem" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%", gap: "1rem" }}>
            <span className="update-banner-text">
              KuramaTracker se ha actualizado!
            </span>
            <div style={{ display: "flex", gap: "0.5rem" }}>
              <button className="update-banner-btn" onClick={handleUpdateApp}>Actualizar</button>
              <button className="update-banner-dismiss" onClick={() => setShowUpdateBanner(false)} aria-label="Cerrar">X</button>
            </div>
          </div>
          <p style={{ fontSize: "0.78rem", color: "rgba(255,255,255,0.65)", margin: 0, lineHeight: "1.4" }}>
            Si el icono de la app sigue siendo el antiguo: borrala de tu pantalla de inicio y vuelvela a instalar desde el navegador.
          </p>
        </div>
      )}
