...
target = """        setFriendData(user);"""

replacement = """        // Fetch anime_points from Supabase as Source of Truth
        try {
...
if target in code:
    code = code.replace(target, replacement, 1) # Only replace first occurrence (which is the main one, wait no, there is an initialization one)
